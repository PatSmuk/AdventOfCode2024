    ...
    while (frequency_iter.next()) |frequency| {
        ...
        for (0..locations.len - 1) |i| {
            ...
            for (1..locations.len - i) |j| {
                ...
                {
                    const x = @as(isize, @intCast(first_location.x)) - dx;
                    const y = @as(isize, @intCast(first_location.y)) - dy;
                ...
                {
                    const x = @as(isize, @intCast(second_location.x)) + dx;
                    const y = @as(isize, @intCast(second_location.y)) + dy;
