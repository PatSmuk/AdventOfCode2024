...
pub fn main() !void {
    ...
    var coords_in_any_path = std.AutoHashMap([2]u15, void).init(allocator);
    ...
    for (paths) |path| {
        for (path) |node| {
            ...
            SearchConfig.fromNode(node, &x, &y, &facing);
            try coords_in_any_path.put(.{ x, y }, {});
        }
    }
    ...
    for (grid, 0..) |row, y| {
        for (row, 0..) |char, x| {
            if (coords_in_any_path.contains(.{ @as(u15, @intCast(x)), @as(u15, @intCast(y)) })) {
                std.debug.print("O", .{});
            } else {
