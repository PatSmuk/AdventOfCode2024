...
pub fn main() !void {
    ...
    for (0..(coords.len - BYTES_TO_SIMULATE)) |i| {
        try corrupted.put(coords[BYTES_TO_SIMULATE + i], {});

        const path = try util.findOnePath(SearchConfig.Node, SearchConfig.Context, SearchConfig.Score, allocator, search_config);
        defer allocator.free(path);

        if (path.len == 0) {
            std.debug.print("{d},{d}", .{ coords[BYTES_TO_SIMULATE + i].x, coords[BYTES_TO_SIMULATE + i].y });
            return;
        }
    }
}
